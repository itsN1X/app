...
export default class Profile extends React.Component {
  ...
  getCoinData = async () => {
    try {
      ...
      this.setState({
        ethPublicKey: coinData[1].publicKey,
        ethPrivateKey: coinData[1].privateKey,
        btcPrivateKey: coinData[0].privateKey,
        btcPublicKey: coinData[0].publicKey,
        loaded: true
      });
    } catch (error) {
      alert(error);
              ...
              <View style={styles.itemContainer}>
                ...
                <View style={styles.addressContainer}>
                  <Text style={styles.addressText}>
                    {this.state.btcPublicKey}
                  </Text>
                  <View style={styles.copyIconContainer}>
                    <TouchableOpacity
                      onPress={() =>
                        this.writeToClipboard(this.state.btcPublicKey)
                      }
                    >
