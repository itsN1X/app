...
export default class ChooseFriends extends React.Component {
  ...
  getNewKeys() {
    ...
    this.setState({
      ...
      old_public_key: this.props.old_public_key,
  ...
  getMnemonic = async () => {
    try {
      ...
      this.setState({
        mnemonic: mnemonic,
        publicKey: mnemonicstr.publicKey,
        username: mnemonicstr.username,
        loaded: true
      });
    } catch (error) {
      console.log(error);
      ...
      axios({
        ...
        .then(function(response) {
          if (response.data.flag === 143) {
            ...
            self.setState({
              friendsPublickey: response.data.result[0].public_key,
              friendsHandle: response.data.result[0].user_name
            });
            self.pushFriendData();
          } else {
  ...
  createTrustData() {
    ...
    for (i = 0; i < friends.length; i++) {
      ...
      data.encrypted_key_data = this.encryptData(
        messageToEncrypt,
        friends[i].address
      );
  ...
  changeShamir(value) {
    if (this.state.friends.length) {
      Alert.alert("Cannot change after adding friends.", "", [
        {
          text: "Continue",
      ...
      return (
        <View style={styles.container}>
            ...
            <View style={{ flex: 1, width: "100%", alignItems: "center" }}>
                ...
                <View>
                  <View style={styles.friendHeadingFlex}>
                    <View style={styles.friendHeadingContainer}>
                      <Text style={styles.friendHeadingText}>
                        Added Devices
                      </Text>
                    </View>

                    <View style={styles.friendNumberContainer}>
                      <Text style={styles.friendNumberText}>
                        {this.state.length}/{this.state.maxDevices}
                      </Text>
                    </View>
                  </View>
                </View>
              ...
              {this.state.friendsAdded ? null : (
                <View style={styles.addButtonFlex}>
                  <TouchableOpacity
                    disabled={this.state.friendsAdded}
              ...
              <View
                style={[styles.friendsContainer, { minHeight: sectionHeight }]}
              >
                <View style={styles.friendsAddedHeadingContainer}>
                  <Text style={styles.friendsAddedHeadingText}>
                    Devices Added for Recovery
                  </Text>
                </View>
                {this.state.friends.map((value, i) => {
                  return (
                    <FriendItem
                      key={value.id}
                      address={value.friendsHandle}
                      id={value.id}
                      onCopy={this.writeToClipboard}
                    />
                  );
                })}
              </View>
          ...
          <View style={styles.saveButtonContainer}>
            {this.state.friendsAdded ? (
              <Button bColor={theme.dark} onPress={this.onSavePress}>
