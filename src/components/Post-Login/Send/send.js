...
export default class Send extends React.Component {
  ...
  calculateUtxo() {
    ...
    if (greaters.length !== 0) {
      greaters.sort(function(a, b) {
        return (
          Unit.fromBTC(a.amount).satoshis - Unit.fromBTC(b.amount).satoshis
        );
      });
  ...
  sendCoins() {
    ...
    if (!bitcoin.Address.isValid(to)) {
      ...
      Toast.showWithGravity(
        "Amount should be lower than spendable amount",
        Toast.LONG,
        Toast.CENTER
      );
    } else if (maxAmount == 0) {
      ...
      this.setState({ loaded: false, activity: "Signing Transaction" }, () => {
        requestAnimationFrame(
          () => this.signTransaction(utxo, amount, from, privateKey, to),
          0
        );
      });
    }
  ...
  signTransaction(utxo, amount, from, privateKey, to) {
    ...
    if (amount + estimatedFees > balance) {
      ...
      Toast.showWithGravity(
        "Amount should be greater than fee",
        Toast.LONG,
        Toast.CENTER
      );
      this.setState({ loaded: true });
      ...
      axios({
        ...
        .then(function(response) {
          if (response.data.flag == 144) {
            Toast.showWithGravity(
              "There is some problem from our end.",
              Toast.LONG,
              Toast.CENTER
            );
            ...
            >
              ...
              <View style={styles.lowerFlex}>
                <KeyboardAvoidingView
                  ...
                  style={styles.inputFlexContainer}
                >
                  <TextInput
