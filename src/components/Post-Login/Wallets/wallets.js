          ...
          if (value === "LoggedIn" || this.props.loggedIn) {
            ...
            this.setState({ activity: "Fetching User Assets" }, () => {
              requestAnimationFrame(
                () => this.getCoinData(wallet_id, user_data),
                0
              );
            });
                  ...
                  {this.state.newData.map((value, i) => {
                    return (
                      <WalletCoinItem
                        ...
                        amount={value.asset_balance || 0}
                        lighticon={value.asset_icon_light}
                        icon={value.asset_icon_dark}
                        name={value.asset_name}
                        currency={this.state.currency}
                        onWalletOpen={this.onWalletOpen}
                      />
                    );
                  })}
                  <View style={{ height: 3 }} />
